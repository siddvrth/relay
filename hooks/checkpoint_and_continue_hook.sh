...
SCRIPT="$PLUGIN_ROOT/skills/checkpoint-and-continue/scripts/context_handoff.py"
TRANSFER_SCRIPT="$PLUGIN_ROOT/skills/checkpoint-and-continue/scripts/transfer_control.py"
    ...
    END {
      for (i=1; i<=length(data); i++) {
        ...
        if (in_string) {
          ...
          if (c=="\"") {
            in_string=0
            j=i+1; while (j<=length(data) && substr(data,j,1) ~ /[[:space:]]/) j++
...
static_fallback() {
  ...
  for candidate in "$state_root"/sessions/*/.active-transfer.json "$state_root"/sessions/*/.revoked.json "$state_root"/sessions/*/transfers/*.json; do
  ...
  if [[ "$source" == "$actor" ]]; then
    ...
    if [[ "$binding_found" != true ]]; then
      for pointer in "$state_root"/sessions/*/transfers/*.json; do
        [[ -f "$pointer" ]] || continue
        pointer_destination="$(sed -nE 's/.*"destination_session_id"[[:space:]]*:[[:space:]]*"([^"]+)".*/\1/p' "$pointer" | head -n1)"
        if [[ "$pointer_destination" == "$actor" ]]; then
          source="$(sed -nE 's/.*"source_session_id"[[:space:]]*:[[:space:]]*"([^"]+)".*/\1/p' "$pointer" | head -n1)"
          binding_found=true
          break
        fi
    ...
    if [[ "$binding_found" != true ]]; then
      for tombstone in "$state_root"/sessions/*/.revoked.json; do
        ...
        if [[ "$tombstone_destination" == "$actor" ]]; then
          source="$(sed -nE 's/.*"source_session_id"[[:space:]]*:[[:space:]]*"([^"]+)".*/\1/p' "$tombstone" | head -n1)"
          binding_found=true
          break
        fi
      done
  ...
  if [[ -f "$tombstone" || ( "$source" != "$actor" && -f "$source_tombstone" ) ]]; then
    implicated=true
  fi
  if [[ -f "$ownership" ]]; then
    ...
    if [[ "$actor" == "$owner_source" || "$actor" == "$owner_destination" ]]; then
      implicated=true
      if [[ "$actor" == "$owner_destination" ]]; then destination_actor=true; else destination_actor=false; fi
    fi
  fi
  ...
  case "$OFFICIAL_EVENT" in
    PreToolUse)
      echo '{"continue":true,"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"transfer runtime unavailable after durable revocation/ownership"}}'
      ;;
    UserPromptSubmit)
      ...
      ;;
    PreCompact|Stop)
      echo '{"continue":false,"stopReason":"transfer runtime unavailable after durable revocation/ownership"}'
      ;;
  esac
}
...
core_pretool_fallback() {
  ...
  set +e
  output="$(python3 "$TRANSFER_SCRIPT" --repo "$ROOT" hook-pretool <<<"$PAYLOAD" 2>/dev/null)"
  status=$?
  set -e
...
case "$EVENT" in
  PreCompact|preCompact|pre-compact)
    ...
    ARGS+=(--trigger pre-compact --reason "Codex plugin PreCompact hook")
    ;;
  Stop|stop)
    OFFICIAL_EVENT="Stop"
    ARGS+=(--trigger stop --reason "Codex plugin Stop hook")
    ;;
  UserPromptSubmit|user-prompt-submit)
    OFFICIAL_EVENT="UserPromptSubmit"
    ARGS+=(--trigger threshold --reason "Codex plugin UserPromptSubmit context check")
    ;;
  ...
  *)
    OFFICIAL_EVENT="Stop"
    ARGS+=(--trigger manual --reason "Codex plugin hook ($EVENT)")
    ;;
esac
